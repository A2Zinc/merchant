...
class Stores extends CI_Model {
	...
	public function store_list($data_filter_arr=array()) {
		...
		if($merchant_id > 0) {
			$this->db->where('store_set.merchant_id',$merchant_id);
		}
	...
	public function count_store_by_merchant_id($merchant_id){
	...
	public function store_delete($store_id) {
		...
		try{
			...
			$this->db->where('store_id',$store_id);
			$result = $this->db->update('store_set');
          	// echo $this->db->last_query();exit;
            
            $respnce['status']  = 'success';
            $respnce['message'] = 'Store successfully deleted';
            return $respnce;
	...
	public function store_add($data){
        try{
            if($this->db->insert('store_set', $data)){
                return TRUE;
            }else{
                return FALSE;
            }
 	...
 	//Update store using API
 	public function store_update_api($data,$store_id) {
		$this->db->where('store_id',$store_id);
		$result = $this->db->update('store_set',$data);
	...
	// Add / Update Tax Information using API
	public function save_tax_information($data) {
		...
		if ($query->num_rows() > 0) {
			...
			// $this->db->where('store_id',$store_id);
			$result = $this->db->update('tax_setting',$data);

		} else {
			$result = $this->db->insert('tax_setting', $data);
		}
	...
	// Add / Update Card Processor Settings using API
	public function save_card_processor_settings($data) {
		...
		if ($query->num_rows() > 0) {
			...
			// $this->db->where('store_id',$store_id);
			$result = $this->db->update('card_transaction_setting',$data);

		} else {
			$result = $this->db->insert('card_transaction_setting', $data);
		}
	...
	// Add / Update Cash Rregister Information using API
	public function save_cash_register_information($data) {
		...
		if ($query->num_rows() > 0) {
			...
			$result = $this->db->insert('web_setting', $data);
		}
