...
class reports extends MY_Model
{
    ...
    public function terminals(){
        $this->db->select("terminal.terminal_id,terminal.mac_address,terminal.username,terminal.active,concat(users.first_name,' ',users.last_name) current_user_name");
      
        $this->db->join('user_login','user_login.username = terminal.username','left');
        $this->db->join('users','users.user_id =user_login.user_id','left');
        
        $terminals=$this->db->from('terminal')->get()->result_array();
        return $terminals;
    }

    public function user_report(){
        $this->db->select("user_login.*,concat(users.first_name,' ',users.last_name) current_user_name, (select daily_hours from timesheet_report where start_date='".date('Y-m-d',strtotime(date('Y-m-d').'-1 day'))."' and timesheet_report.username=user_login.username) daily_hours");
        $this->db->join('users','users.user_id =user_login.user_id','left');
            
        $users=$this->db->from('user_login')->get()->result_array();
        return $users;   
    }

    public function best_selling_products($type=''){
        switch($type){
            case "1":
            $where=['created_at>=' => date("Y-m-d",strtotime("-1 month")),'created_at<=' => date('Y-m-d')];
            break;

            case "3":
            $where=['created_at>=' => date("Y-m-d",strtotime("-3 months")),'created_at<=' => date('Y-m-d')];
            break;

            case "6":
            $where=['created_at>=' => date("Y-m-d",strtotime("-6 months")),'created_at<=' => date('Y-m-d')];
            break;

            case "12":
            $where=['created_at>=' => date("Y-m-d",strtotime("-12 months")),'created_at<=' => date('Y-m-d')];
            break;

            default:
            $where=['1' => 1];
        }
        $products=$this->db->select('product_id,product_name,count(*) sale_count')->where($where)->from('order_details')->group_by('product_id')->limit(10)->order_by('sale_count','desc')->get()->result_array();
        return $products;   
    }
     public function last_sales(){
        $this->db->select("order.*,tbl_user_shift.shift_no");
        $this->db->join('tbl_user_shift',"tbl_user_shift.id=order.shift","left");
        $orders=$this->db->from('order')->limit(10)->order_by('id','desc')->get()->result_array();
        foreach ($orders as $skey => $svalue) {
            $order_details=$this->Reports->get_order_details($svalue['order_id']);
            $orders[$skey]['order_details']=$order_details;
        }
    ...
    public function get_order_details($order_id){
        $order_details=$this->db->from('order_details')->where(['order_id' => $order_id])->get()->result_array();
...
//=============sales report store wise=================
    ...
    public function grand_total_sales_report()
    {
