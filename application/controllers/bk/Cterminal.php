...
class Cterminal extends CI_Controller {
	...
	public function add_store_terminal() {
        ...
        $content = $this->parser->parse('store_terminal/add_terminal',$data,true);
        $this->template->full_admin_html_view($content);
   	}
	...
	public function edit_store_terminal() {
   	...
   	// Generate License
	public function generate_license($merchant_id=0,$store_id=0) {
		...
		if($merchant_id > 0 && $store_id > 0) {
			$license_key = hash('ripemd160', $merchant_id."_".$store_id);
			$resp['status'] 	 = 1;
			$resp['license_key'] = $license_key;
		}
   	...
   	// Store Terminal Add
	public function add_store_terminal_db() {

	  	// Terminal information add
		$merchant_id = $this->input->post('merchant_id');
		$store_id 	 = $this->input->post('store_id');
		...
		$result=$this->Terminals->add_store_terminal_db($param);
		if ($result) {
			$this->session->set_flashdata('success', "Terminal is successfully added");
	...
	public function get_store_basedon_merchant() {
		$merchant_id = !empty($this->input->post('merchant_id')) ? $this->input->post('merchant_id') : 0;
		...
		if($merchant_id > 0) {			
	        ...
	        if(!empty($store_list)) {
	        	foreach ($store_list as $key_s => $value_s) {
	        		if($value_s["store_id"] == $store_id)
    					$html .='<option value="'.$value_s["store_id"].'" selected="selected">'.$value_s["store_name"].'</option>';
    				else
    					$html .='<option value="'.$value_s["store_id"].'">'.$value_s["store_name"].'</option>';
	        	}
	        }
