...
class Ctax extends CI_Controller {
    ...
    {
        $data['title'] = "All Tax";
        $data['tax'] = $this->Taxs->get_tax();
        //echo '<pre>'; print_r($data);exit;
        $content = $this->parser->parse('tax/tax',$data,true);
        $this->template->full_admin_html_view($content);
    }
    ...
    public function updateTax(){
        ...
        //$tax_id = 1;
        $update = $this->Taxs->updateTax($data);
        if($update == 1) {
            $this->session->set_flashdata('success', "Tax is Successfully Updated.");
            redirect('Ctax/manage_tax');
        } else {
