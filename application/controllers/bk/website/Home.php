    ...
    public function submit_checkout()
    {
        ...
        if ($payment_method == 1) {
            ...
            if ($result) {
                ...
                $this->session->set_userdata("order_completed_number", "$order_id");
                $this->cart->destroy();
...
//        return $product;
    ...
    public function prodctDatas(){
        $categoryId = $this->input->post('id');
        $productDatacategory =  $this->db->select('*')->where('category_id',$categoryId)
                                            ->limit(3)
                                            ->get('product_information')
                                            ->result_array();
       echo json_encode($productDatacategory);
	   die;
    }
	public function thankyou(){
		$content = $this->lhome->thankyou_page();
		$this->template->full_website_html_view($content);
	}
}
