...
class Merchant_dashboard extends CI_Controller {
	...
	public function index(){
		...
		$stores=$this->Stores->store_list($data_filter_arr);
		$total_product 		= 0;
		$sales_report 		= 0;
		$grand_total_sales	= 0;

		foreach ($stores as $key => $value) {
			$this->Products->set_db($value['store_db']);
			$total_product+= (float)$this->Products->count_product();
			$this->Reports->set_db($value['store_db']);
			$sales_report+= (float)$this->Reports->todays_total_sales_report();
			$grand_total_sales+= (float)$this->Reports->grand_total_sales_report();
		}
		$this->Products->set_db();
